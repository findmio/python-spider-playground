...
function get_m() {
    ...
    function md5_cmn(h, e, d, c, g, f) {
        return safe_add(
            bit_rol(safe_add(safe_add(e, h), safe_add(c, f)), g),
            d
        );
    }
    ...
    var timestamp = Date.parse(new Date()) + 1e8;

    const f = hex_md5(timestamp.toString());

    const m = f + "\u4e28" + timestamp / (-1 * 3483 + -9059 + 13542);
